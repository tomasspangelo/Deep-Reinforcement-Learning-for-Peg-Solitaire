...
def init_critic(critic_config, state_shape):
    ...
    critic = NNCritic(keras_model)
    return critic


def init_learner(learner_config, actor, critic, sim_world):
    decay_factor = float(learner_config['decay_factor'])
...
def main():
    ...
    learner = init_learner(config['learner'], actor, critic, sim_world)
